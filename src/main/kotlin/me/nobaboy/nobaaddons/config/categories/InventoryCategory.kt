...
object InventoryCategory {
	fun create(defaults: NobaConfig, config: NobaConfig) = NobaConfigUtils.buildCategory(tr("nobaaddons.config.inventory", "Inventory")) {
		...
		buildGroup(tr("nobaaddons.config.inventory.pickupLog", "Item Pickup Log")) {
			...
			slider(
				tr("nobaaddons.config.inventory.pickupLog.timeout", "Expire After"),
				min = 2,
				max = 10,
				step = 1,
			...
			cycler(
				tr("nobaaddons.config.inventory.pickupLog.style", "Text Style"),
				default = TextShadow.SHADOW,
				property = UISettings.pickupLog::textShadow
			)
		}
		// endregion
