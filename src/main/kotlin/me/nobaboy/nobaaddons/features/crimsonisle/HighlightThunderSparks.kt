...
object HighlightThunderSparks {
	private val config get() = NobaConfigManager.get().crimsonIsle.highlightThunderSparks
	...
	private fun renderHighlights(context: WorldRenderContext) {
		...
		sparks.forEach {
			...
			val block = vec.getBlockStateAt()
			val throughBlocks = vec.distanceToPlayer() < 6 && block.fluidState != null && block.fluidState.fluid is LavaFluid
			RenderUtils.drawOutlinedFilledBox(context, vec.add(x = -0.5, z = -0.5), color, extraSize = -0.25, throughBlocks = throughBlocks)
