...
object HoppityEggGuess {
	...
	private fun renderWaypoints(context: WorldRenderContext) {
		...
		guessLocation?.let {
			...
			val textLocation = it.center()

			RenderUtils.renderWaypoint(context, it, NobaColor.AQUA, throughBlocks = true)
			RenderUtils.renderText(context, textLocation, tr("nobaaddons.events.hoppity.eggGuessWaypoint", "Egg Guess"), color = NobaColor.AQUA, yOffset = -10f, throughBlocks = true)

			if(distance > 5) {
				val formattedDistance = distance.toInt().addSeparators()
				RenderUtils.renderText(context, textLocation, "${formattedDistance}m", color = NobaColor.GRAY, hideThreshold = 5.0, throughBlocks = true)
			}
		}
	}
